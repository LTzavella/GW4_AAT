...
fun_AAT_RTs <- function(x) {
  c(
    Pre.Pull.Filler.RT = median(x$total_rt[x$condition == "pre_pull_filler"]),
...
fun_AAT_ERs <- function(x) {
  c(
    Pre.Pull.Filler.ER = mean(x$accuracy[x$condition == "pre_pull_filler"]),
...
fun_ratings <- function(x) {
  c(
    Pre.Liking.Go = mean(x$values.rating[x$blockcode == "pre_ratings" & (x$food_category == "go_sweet" | x$food_category == "go_savoury")]),
...
fun_gng_RTs <- function(x) {
  c(
    FillerGo.RT = mean(x$values.gng_rt[(x$values.gng_trial_type == "filler_go_left" | x$values.gng_trial_type == "filler_go_right")]),
...
fun_gng_PEs <- function(x) {
  c(
    FillerGo.PE = mean(x$accuracy[(x$values.gng_trial_type == "filler_go_left" | x$values.gng_trial_type == "filler_go_right")]),
