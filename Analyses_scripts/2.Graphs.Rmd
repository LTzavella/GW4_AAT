...
required.packages <- c("cowplot", "dplyr","reshape2", "ggplot2", "ggpubr", "viridis","here",
...
ggsave('graphs/test_retest.png', dpi = 300, width = 14, height=6)
```

```{r}
ggplot(split, aes(y=Pre_Bias_Odd, x=Pre_Bias_Even)) + 
...
cor.test(split, x = split$Pre_Bias_Odd, y=split$Pre_Bias_Even, alternative = "greater", method = "pearson")
```
...
geom_point(data = gng_graphs, aes(x = as.numeric(as.factor(condition))+.2, y = PCsignal, group = condition, colour =  condition), alpha=0.6, size=1.25, show.legend = FALSE, position = position_jitter(width = 0.05))+
  labs(x="", y="Proportion of correct responses on signal trials")+ theme(legend.text=element_text(size=18)) +
    theme(axis.title.y=element_blank(),
        ...
        axis.ticks.y=element_blank()) + coord_flip() + scale_x_discrete(position = "left")
...
ggsave('graphs/liking_raincloud.png', dpi = 300, width = 10, height=7)
```
